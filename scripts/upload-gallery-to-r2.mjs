...
function getReferencedGalleryFiles() {
  ...
  for (const match of content.matchAll(/\/assets\/gallery\/[^"']+/g)) {
    const rel = match[0].slice(1);
    relativePaths.add(rel);
    const thumbRel = rel.replace("assets/gallery/", "assets/gallery/thumbs/");
    if (fs.existsSync(path.join(repoRoot, thumbRel))) {
      relativePaths.add(thumbRel);
    }
  }
