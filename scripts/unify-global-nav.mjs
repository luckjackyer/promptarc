...
function prettyRoute(file) {
  const route = rel(file).replace(/index\.html$/, "");
  return route ? `/${route}` : "/";
}

function languageHref(file, zh) {
  const route = prettyRoute(file);
  if (zh) {
    if (route.startsWith("/zh/generate")) return "/generate/";
    if (route === "/zh/home-next/") return "/";
    if (route === "/zh/admin/members/") return "/account/";
    return route.replace(/^\/zh\//, "/");
  }
  if (route === "/generate/") return "/zh/generate-image-first/";
  return `/zh${route}`;
}
...
function headerFor(file) {
  ...
  const langHref = languageHref(file, zh);
  const labels = zh
    ? {
        home: "PromptArc \u9996\u9875",
        nav: "\u4e3b\u5bfc\u822a",
...
${navItem({ key: "home", href: homeHref, label: labels.homeNav, current })}
...
${navItem({ key: "account", href: `${base}/account/`, label: labels.account, current })}
      ...
      <a class="site-global-lang" href="${langHref}" aria-label="${labels.lang}: ${labels.langTarget}">
