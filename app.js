(function () {
  ...
  function copyText(targetSelector, triggerButton) {
    ...
    const showCopyFeedback = function (message) {
      if (!button) {
        return;
      }
    ...
    copyPlainText(target.textContent || "")
      .then(() => {
        showCopyFeedback(i18n.copied);
      })
      .catch(() => {
        showCopyFeedback(i18n.copyFailed);
      });
  }
  ...
  function redirectToLogin(returnTo) {
    ...
    const loginPath = isChinese ? "/zh/account/login/" : "/account/login/";
    window.location.href = loginPath + "?returnTo=" + encodeURIComponent(target);
  }
  ...
  function buildGeneratorUrlFromHistory(item) {
    const basePath = isChinese ? "/zh/generate-image-first/" : "/generate/";
  ...
  function initCollectionExplorer(options) {
    ...
    const searchInputs = Array.from(document.querySelectorAll(options.searchSelector));
    ...
    function apply() {
      ...
      document.dispatchEvent(
        new CustomEvent("promptarc:collection-filter-change", {
          detail: {
            filter: activeFilter,
            categoryOnly: activeCategoryOnly,
            query: query
          }
        })
      );
    ...
    searchInputs.forEach((input) => {
      input.addEventListener("input", function () {
        ...
        searchInputs.forEach((peer) => {
          if (peer !== input && peer.value !== searchQuery) {
            peer.value = searchQuery;
          }
        });
        apply();
      });
    ...
    document.querySelectorAll("[data-email-gate]").forEach((form) => {
      ...
      form.addEventListener("submit", function (event) {
        ...
        if (submitButton) {
          ...
          submitButton.textContent = isChinese ? "提交中" : "Submitting";
        }

        const resetSubmit = function () {
          if (submitButton) {
            submitButton.disabled = false;
            submitButton.textContent = idleLabel || (isChinese ? "提交" : "Submit");
          }
        };
          ...
          fetch(config.newsletterEndpoint, {
            ...
            .catch(() => {
              if (feedback) {
                feedback.textContent = hasDownload
                  ? isChinese
                    ? "邮件提交失败，但你仍可下载样本。请稍后再试一次。"
                    : "The email signup failed, but you can still download the sample. Please try again later."
                  : isChinese
                    ? "名单接口暂时不可用，请稍后再试。"
  ...
  function initImageGeneratorPrep() {
    ...
    function renderMockGenerationResult() {
      const mockResultValue = isLocalPreview ? params.get("mock-result") : "";
    ...
    form.addEventListener("submit", async function (event) {
      ...
      if (document.body && document.body.getAttribute("data-page") === "home-canvas") {
        const targetUrl = new URL(isChinese ? "/zh/generate-image-first/" : "/generate/", window.location.origin);
      ...
      for (let index = 0; index < requestedCount; index += 1) {
        ...
        if (resultStrip) {
          resultStrip.insertAdjacentHTML(
            "beforeend",
            '<div class="generator-result-thumb generator-result-slot is-generating" role="status" data-generator-slot-state="generating" data-generated-index="' + index + '"><span>' + (isChinese ? "第 " + (index + 1) + " 张生成中" : "Image " + (index + 1) + " generating") + "</span></div>"
          );
  ...
  function initGeneratedResultPreview() {
    ...
    function ensureModal() {
      ...
      modal.addEventListener("click", function (event) {
        ...
        const remixButton = event.target.closest("[data-remix-generated]");
        if (remixButton && modal) {
          handleGeneratedPreviewRemix(modal);
        }
        const regenerateButton = event.target.closest("[data-regenerate-current]");
        if (regenerateButton && modal) {
          handleGeneratedPreviewRegenerate(modal, regenerateButton);
        }
      });
      document.addEventListener("keydown", function (event) {
    ...
    function handleGeneratedPreviewRemix(preview) {
      const candidate = (preview.generatedPreviewCandidates || [])[preview.generatedPreviewActiveIndex || 0];
      const promptInput = document.querySelector('form[data-generate-form] textarea[name="prompt"], form.generate-composer-card textarea[name="prompt"]');
      if (candidate && candidate.prompt && promptInput) {
        promptInput.value = candidate.prompt;
        promptInput.focus();
    ...
    function handleGeneratedPreviewRegenerate(preview, triggerButton) {
      handleGeneratedPreviewRemix(preview);
      const generateForm = document.querySelector("form[data-generate-form], form.generate-composer-card");
      if (!generateForm) {
        return;
      }
      if (triggerButton) {
        triggerButton.disabled = true;
        triggerButton.textContent = isChinese ? "生成中" : "Generating";
      }
      ...
      window.setTimeout(function () {
        if (triggerButton) {
          ...
          triggerButton.textContent = isChinese ? "\u518d\u6b21\u751f\u6210" : "Generate again";
        }
      }, 1200);
    }
    ...
    if (isLocalPreview && previewParams.get("mock-result") && /detailmodal|detail/.test(String(previewParams.get("verify") || ""))) {
      window.setTimeout(function () {
    ...
    grid.addEventListener("click", function (event) {
      ...
      if (config.imageGeneratorEndpoint) {
        fetch(config.imageGeneratorEndpoint, {
          ...
          .then((response) => response.json().catch(() => ({ ok: false, error: "gallery_submission_failed" })).then((payload) => ({ response, payload })))
          .then(({ response, payload }) => {
            if (!response.ok || !payload.ok || !payload.submitted) {
              throw new Error("gallery_submission_failed");
            }
            submitButton.textContent = isChinese ? "已提交审核" : "Submitted";
          })
          .catch(() => {
            submitButton.disabled = false;
            submitButton.textContent = isChinese ? "提交失败" : "Submit failed";
            window.setTimeout(() => {
              submitButton.textContent = originalText || (isChinese ? "提交精选" : "Submit");
            }, 1600);
          });
      } else {
        submitButton.disabled = false;
  ...
  function initAccountPages() {
    ...
    function getReturnTo() {
      const params = new URLSearchParams(window.location.search);
      return params.get("returnTo") || (isChinese ? "/zh/generate-image-first/" : "/generate/");
    }

    function renderAccountHistoryItems(items) {
      if (!historyNode) return;
      if (!items || !items.length) {
        renderAccountHistoryState(
          isChinese ? "\u8fd8\u6ca1\u6709\u751f\u6210\u8bb0\u5f55" : "No generated images yet",
          isChinese
            ? "\u5f00\u59cb\u751f\u6210\u540e\uff0c\u8fd9\u91cc\u4f1a\u663e\u793a\u56fe\u7247\u3001\u63d0\u793a\u8bcd\u3001\u6bd4\u4f8b\u548c\u6a21\u578b\u4fe1\u606f\u3002"
            : "After generation starts, images, prompts, ratios, and model details will appear here."
        );
        return;
      }
    ...
    function renderAccountHistoryState(title, body) {
      if (!historyNode) return;
      historyNode.innerHTML = [
        '<p class="account-empty-state">',
    ...
    async function loadAccountHistory() {
      if (!historyNode) return;
      renderAccountHistoryState(
        isChinese ? "\u6b63\u5728\u52a0\u8f7d\u751f\u6210\u5386\u53f2" : "Loading generated history",
        isChinese ? "\u6b63\u5728\u540c\u6b65\u4f60\u7684\u56fe\u7247\u3001\u63d0\u793a\u8bcd\u548c\u751f\u6210\u53c2\u6570\u3002" : "Syncing your images, prompts, and generation parameters."
      );
      try {
        ...
        if (!response.ok || !payload.ok) {
          renderAccountHistoryState(
            isChinese ? "\u6682\u65f6\u65e0\u6cd5\u52a0\u8f7d\u5386\u53f2" : "History could not be loaded",
            isChinese ? "\u8bf7\u7a0d\u540e\u518d\u8bd5\uff0c\u6216\u56de\u5230\u751f\u6210\u5668\u7ee7\u7eed\u751f\u6210\u56fe\u7247\u3002" : "Try again later, or return to the generator and keep creating images."
          );
          return;
        }
      ...
      loginForm.addEventListener("submit", async function (event) {
        ...
        try {
          ...
          if (payload.debugToken) {
            const loginPath = isChinese ? "/zh/account/login/" : "/account/login/";
            const debugUrl = loginPath + "?token=" + encodeURIComponent(payload.debugToken) + "&returnTo=" + encodeURIComponent(getReturnTo());
    ...
    loadPromptArcSession().then(function (session) {
      if (!session || !session.authenticated) {
        ...
        if (historyNode) {
          renderAccountHistoryState(
            isChinese ? "\u767b\u5f55\u540e\u540c\u6b65\u751f\u6210\u5386\u53f2" : "Log in to sync generation history",
            isChinese
              ? "\u767b\u5f55\u540e\uff0c\u8fd9\u91cc\u4f1a\u663e\u793a\u751f\u6210\u56fe\u7247\u3001\u63d0\u793a\u8bcd\u3001\u6bd4\u4f8b\u548c\u6a21\u578b\u4fe1\u606f\u3002"
              : "After login, generated images, prompts, ratios, and model details will appear here."
          );
        }
        if (view !== "login") {
      ...
      if (historyNode) {
        renderAccountHistoryState(
          isChinese ? "\u6b63\u5728\u51c6\u5907\u751f\u6210\u5386\u53f2" : "Preparing generation history",
          isChinese ? "\u751f\u6210\u5386\u53f2\u63a5\u53e3\u8fd4\u56de\u540e\uff0c\u8fd9\u91cc\u4f1a\u663e\u793a\u4f60\u7684\u56fe\u7247\u3002" : "Your images will appear here after the history endpoint responds."
        );
      }
  ...
  function initGallery() {
    ...
    const shouldBatchGallery = !isHomeFeatured && !(Number.isFinite(galleryLimit) && galleryLimit > 0);
    const galleryBatchSize = window.matchMedia && window.matchMedia("(max-width: 700px)").matches ? 18 : 30;
    let visibleItems = fullSet.slice();
    let renderedCount = shouldBatchGallery ? galleryBatchSize : Infinity;
    let sortMode = "latest";
    let savedOnly = false;
    let homeFeaturedCycle = 0;
    let collectionFilterState = {
      filter: initialParams.get("category") || initialParams.get("filter") || "all",
      categoryOnly: Boolean(initialParams.get("category")),
      query: ""
    };
    const savedPrompts = new Set(JSON.parse(window.localStorage.getItem("promptarc:saved-prompts") || "[]"));
    const loadMoreButton = shouldBatchGallery ? document.createElement("button") : null;
    ...
    function applyCurrentView() {
      const filteredSet = initialTagFilter
        ...
        : fullSet;
      const filterValue = String(collectionFilterState.filter || "all").toLowerCase();
      const query = String(collectionFilterState.query || "").trim().toLowerCase();
      const collectionFilteredSet = filteredSet.filter((item) => {
        const tagText = (item.tags || []).join(" ").toLowerCase();
        const haystack = [item.title, item.category, tagText, item.prompt].join(" ").toLowerCase();
        const matchesFilter =
          filterValue === "all" ||
          String(item.category || "").toLowerCase() === filterValue ||
          (!collectionFilterState.categoryOnly && tagText.includes(filterValue));
        const matchesSearch = !query || haystack.includes(query);
        return matchesFilter && matchesSearch;
      });
      const sourceItems = savedOnly ? collectionFilteredSet.filter((item) => savedPrompts.has(item.id)) : collectionFilteredSet.slice();
      if (sortMode === "oldest") {
    ...
    function syncGalleryControls() {
      ...
      if (loadMoreButton) {
        const remaining = Math.max(visibleItems.length - Math.min(renderedCount, visibleItems.length), 0);
        loadMoreButton.hidden = remaining <= 0;
        loadMoreButton.textContent = isChinese ? "加载更多图片（还剩 " + remaining + "）" : "Load more images (" + remaining + " left)";
      }
    }

    function resetGalleryBatch() {
      renderedCount = shouldBatchGallery ? galleryBatchSize : Infinity;
    }
    ...
    function renderGallery() {
      ...
      const selectedItems = getHomeFeaturedItems(visibleItems, galleryLimit);
      const renderItems = shouldBatchGallery ? selectedItems.slice(0, renderedCount) : selectedItems;
      ...
      card.innerHTML = [
        ...
        '<img src="' + getThumbnailUrl(item.imageUrl) + '" data-full-src="' + getGalleryImageUrl(item.imageUrl) + '" alt="' + item.title + " " + i18n.imageAltSuffix + '" loading="' + (itemIndex < 10 ? "eager" : "lazy") + '" decoding="async" fetchpriority="' + (itemIndex < 4 ? "high" : "auto") + '" data-gallery-image="true">',
        "</button>",
        "</div>",
    ...
    if (loadMoreButton) {
      ...
      loadMoreButton.setAttribute("data-gallery-load-more", "true");
      grid.insertAdjacentElement("afterend", loadMoreButton);
      loadMoreButton.addEventListener("click", function () {
        renderedCount += galleryBatchSize;
        renderGallery();
      });
    }
    renderGallery();
    ...
    document.addEventListener("promptarc:collection-filter-change", function (event) {
      const detail = (event && event.detail) || {};
      const nextState = {
        filter: detail.filter || "all",
        categoryOnly: Boolean(detail.categoryOnly),
        query: detail.query || ""
      };
      if (
        nextState.filter === collectionFilterState.filter &&
        nextState.categoryOnly === collectionFilterState.categoryOnly &&
        nextState.query === collectionFilterState.query
      ) {
        return;
      }
      collectionFilterState = nextState;
      resetGalleryBatch();
      renderGallery();
    });
  ...
  function initGenerateNext() {
    ...
    const params = new URLSearchParams(window.location.search);
    if (isLocalPreview && params.get("mock-result")) {
  ...
  function initGenerateImageFirst() {
    ...
    const params = new URLSearchParams(window.location.search);
    const isMockPreview = isLocalPreview && (params.has("mock-result") || params.has("detail") || params.has("verify"));
    ...
    const historyPrompts = isChinese ? [
      ...
      "Unreal Engine 5 render, extreme top-down view, a white-robed immortal figure in layered silk standing on the roof terrace of an ancient palace, no visible sky, looking across a glowing city that stretches toward the horizon, with the curved surface of a planet and oceans of lights below the clouds.",
      "Unreal Engine 5 render, extreme top-down view, a long-haired immortal figure in ornate white robes standing on an ancient palace roof, no visible sky, distant city light bands wrapping across the curve of the planet."
    ];
    ...
    const imageFirstText = {
      candidate: isChinese ? "候选 " : "Candidate ",
      imageUnit: isChinese ? " 张" : " images",
      stable: isChinese ? "稳定复现" : "Stable repeat",
      subtle: isChinese ? "轻微变化" : "Light variation",
      strong: isChinese ? "明显变化" : "Strong variation",
      emptyAria: isChinese ? "未生成图片" : "No generated images yet",
      emptyLabel: isChinese ? "图片生成" : "Image generation",
      emptyTitle: isChinese ? "从一句提示词开始，生成第一组图像" : "Start with one prompt and generate your first image set.",
      emptyBody: isChinese
        ? "输入想法后，我们会把结果按轮次留在页面里，方便你比较版本、回到方向，继续往下生成。"
        : "Enter an idea and PromptArc keeps each generated round on the page so you can compare versions, return to a direction, and keep generating.",
      justGenerated: isChinese ? "刚刚生成" : "Just generated",
      previousRound: isChinese ? "上一轮生成" : "Previous round",
      editAgain: isChinese ? "重新编辑" : "Edit prompt",
      generateAgain: isChinese ? "再次生成" : "Generate again",
      generatedAlt: isChinese ? "生成候选图 " : "Generated candidate ",
      thumbnailAlt: isChinese ? " 缩略图" : " thumbnail"
    };
    ...
    function getImageFirstState() {
      ...
      const variationLabel = variationValue === "stable" ? imageFirstText.stable : variationValue === "strong" ? imageFirstText.strong : variationValue === "subtle" ? imageFirstText.subtle : String(variationValue || imageFirstText.subtle);
    ...
    function updateImageFirstSettingsSummary() {
      ...
      settingsSummary.textContent = state.modelLabel + " / " + state.ratioLabel + " / " + state.count + imageFirstText.imageUnit + " / " + state.resolution;
    }
    ...
    function setImageFirstDetail(candidate, index, group) {
      ...
      if (detailTitle) detailTitle.textContent = imageFirstText.candidate + (index + 1);
      ...
      if (detailThumbs) {
        ...
        detailThumbs.innerHTML = group.candidates.map(function (item, itemIndex) {
          return '<button type="button" class="' + (itemIndex === index ? "is-selected" : "") + '" data-image-first-detail-thumb="' + itemIndex + '"><img src="' + escapeHtml(item.imageUrl) + '" alt="' + escapeHtml(imageFirstText.candidate + (itemIndex + 1) + imageFirstText.thumbnailAlt) + '"></button>';
        }).join("");
      }
    }
    ...
    function selectImageFirstCandidate(groupId, index) {
      ...
      if (title) title.textContent = imageFirstText.candidate + (index + 1);
    ...
    function renderImageFirstEmptyState() {
      ...
      results.innerHTML = [
        '<section class="image-first-empty-state" aria-label="' + escapeHtml(imageFirstText.emptyAria) + '">',
        '<div class="image-first-empty-copy">',
        "<span>" + escapeHtml(imageFirstText.emptyLabel) + "</span>",
        "<h1>" + escapeHtml(imageFirstText.emptyTitle) + "</h1>",
        "<p>" + escapeHtml(imageFirstText.emptyBody) + "</p>",
        "</div>",
    ...
    function renderImageFirstGroup(group, index) {
      const actionLabel = index === 0 ? imageFirstText.justGenerated : imageFirstText.previousRound;
      return [
        ...
        '<button type="button" data-image-first-edit-group="' + escapeHtml(group.id) + '">' + escapeHtml(imageFirstText.editAgain) + '</button>',
        '<button type="button" data-image-first-regenerate-group="' + escapeHtml(group.id) + '">' + escapeHtml(imageFirstText.generateAgain) + '</button>',
        '</div>',
        ...
        group.candidates.map(function (candidate, candidateIndex) {
          return '<button type="button" data-image-first-group="' + escapeHtml(group.id) + '" data-image-first-candidate="' + candidateIndex + '"><img src="' + escapeHtml(candidate.imageUrl) + '" alt="' + escapeHtml(imageFirstText.generatedAlt + (candidateIndex + 1)) + '"></button>';
        }).join(""),
        "</div>",
        "</article>"
      ].join("");
    ...
    async function regenerateImageFirstGroup(group, triggerButton) {
      if (!group) return;
      setPromptFromImageFirstGroup(group);
      if (isMockPreview || !config.imageGeneratorEndpoint) {
      ...
      try {
        ...
        if (triggerButton) {
          triggerButton.disabled = false;
          triggerButton.textContent = originalText || (isChinese ? "再次生成" : "Generate again");
        }
      }
    }
    ...
    results.addEventListener("click", function (event) {
      ...
      if (regenerateButton) {
        regenerateImageFirstGroup(getImageFirstGroup(regenerateButton.getAttribute("data-image-first-regenerate-group") || ""), regenerateButton);
        return;
      }
    ...
    const mockCount = isLocalPreview ? Math.min(Math.max(parseInt(String(params.get("mock-result") || ""), 10) || 0, 0), 4) : 0;
  ...
  function initAdminMembersPage() {
    ...
    function renderMessage(message, tone) {
      root.innerHTML = [
        '<div class="admin-members-empty' + (tone ? " is-" + escapeHtml(tone) : "") + '">',
        "<strong>" + escapeHtml(message) + "</strong>",
        "<p>" +
          escapeHtml(
            isChinese
              ? "请使用带管理员令牌的链接进入。验证通过后，这里会显示会员邮箱、套餐、额度使用和生成权限。"
              : "Open this page with an admin token. After verification, this view shows member email, plan status, quota usage, and generation access."
          ) +
          "</p>",
        "</div>"
      ].join("");
    ...
    function renderMembers(members) {
      if (!members || !members.length) {
        renderMessage(isChinese ? "暂无会员数据" : "No members yet.", "empty");
        return;
      }
      root.innerHTML = [
        '<div class="admin-members-table-head"><span>' +
          escapeHtml(isChinese ? "会员" : "Member") +
          "</span><span>" +
          escapeHtml(isChinese ? "额度" : "Quota") +
          "</span></div>",
    ...
    if (!adminToken) {
      renderMessage(isChinese ? "需要管理员权限" : "Admin access is required.", "locked");
      return;
    }

    renderMessage(isChinese ? "正在加载会员列表" : "Loading members.", "loading");
    fetch("/api/admin/members", {
      ...
      .then(function (result) {
        if (!result.response.ok || !result.payload.ok) {
          renderMessage(isChinese ? "管理员权限校验失败" : "Admin access check failed.", "locked");
          return;
        }
        renderMembers(result.payload.members || []);
      })
      .catch(function () {
        renderMessage(isChinese ? "暂时无法加载会员列表" : "Members could not be loaded.", "error");
      });
  }
