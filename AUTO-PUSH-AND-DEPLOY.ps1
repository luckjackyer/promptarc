...
function Invoke-MojibakeAudit {
  ...
  $auditScript = Join-Path $repoRoot "scripts\audit-chinese-mojibake.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
}

function Invoke-TrustSignalAudit {
  param(
    [string]$Label,
    [string]$TargetPath
  )
  ...
  Write-Step $Label
  $nodePath = Get-NodePath
  $auditScript = Join-Path $repoRoot "scripts\audit-trust-signals.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
...
function Invoke-PricingCreditAudit {
  param(
    [string]$Label,
    [string]$TargetPath
  )
  ...
  Write-Step $Label
  $nodePath = Get-NodePath
  $auditScript = Join-Path $repoRoot "scripts\audit-pricing-credits.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
...
function Invoke-CommercialLinkAudit {
  param(
    [string]$Label,
    [string]$TargetPath
  )
  ...
  Write-Step $Label
  $nodePath = Get-NodePath
  $auditScript = Join-Path $repoRoot "scripts\audit-commercial-links.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
...
function Invoke-NumberedPromptProductAudit {
  param(
    [string]$Label,
    [string]$TargetPath
  )
  ...
  Write-Step $Label
  $nodePath = Get-NodePath
  $auditScript = Join-Path $repoRoot "scripts\audit-numbered-prompt-product.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
...
function Invoke-SeoClusterAudit {
  param(
    [string]$Label,
    [string]$TargetPath
  )
  ...
  Write-Step $Label
  $nodePath = Get-NodePath
  $auditScript = Join-Path $repoRoot "scripts\audit-seo-clusters.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
...
function Invoke-TopicAnswerPageAudit {
  param(
    [string]$Label,
    [string]$TargetPath
  )
  ...
  Write-Step $Label
  $nodePath = Get-NodePath
  $auditScript = Join-Path $repoRoot "scripts\audit-topic-answer-pages.mjs"
  & $nodePath $auditScript $TargetPath
  if ($LASTEXITCODE -ne 0) {
    Fail "$Label failed."
  }
...
function Build-LightweightDeployRepo {
  ...
  Invoke-MojibakeAudit -Label "Checking deploy artifact Chinese text" -TargetPath $DeployRoot
  Invoke-TrustSignalAudit -Label "Checking deploy artifact trust signals" -TargetPath $DeployRoot
  Invoke-PricingCreditAudit -Label "Checking deploy artifact credit pricing page" -TargetPath $DeployRoot
  Invoke-CommercialLinkAudit -Label "Checking deploy artifact commercial links" -TargetPath $DeployRoot
  Invoke-NumberedPromptProductAudit -Label "Checking deploy artifact numbered prompt product" -TargetPath $DeployRoot
  Invoke-SeoClusterAudit -Label "Checking deploy artifact SEO clusters" -TargetPath $DeployRoot
  Invoke-TopicAnswerPageAudit -Label "Checking deploy artifact topic answer pages" -TargetPath $DeployRoot
...
try {
  ...
  Invoke-MojibakeAudit -Label "Checking source Chinese text" -TargetPath $repoRoot
  Invoke-TrustSignalAudit -Label "Checking source trust signals" -TargetPath $repoRoot
  Invoke-PricingCreditAudit -Label "Checking source credit pricing page" -TargetPath $repoRoot
  Invoke-CommercialLinkAudit -Label "Checking source commercial links" -TargetPath $repoRoot
  Write-Step "Building numbered prompt catalog"
  $nodePath = Get-NodePath
  & $nodePath "scripts\build-numbered-prompt-catalog.mjs"
  if ($LASTEXITCODE -ne 0) {
    Fail "Building numbered prompt catalog failed."
  }
  Invoke-NumberedPromptProductAudit -Label "Checking source numbered prompt product" -TargetPath $repoRoot
  Write-Step "Reinforcing SEO clusters"
  & $nodePath "scripts\reinforce-seo-clusters.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Reinforcing SEO clusters failed."
  }
  Write-Step "Reinforcing topic answer pages"
  & $nodePath "scripts\reinforce-topic-answer-pages.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Reinforcing topic answer pages failed."
  }
  Invoke-SeoClusterAudit -Label "Checking source SEO clusters" -TargetPath $repoRoot
  Invoke-TopicAnswerPageAudit -Label "Checking source topic answer pages" -TargetPath $repoRoot
