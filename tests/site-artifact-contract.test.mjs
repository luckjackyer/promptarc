...
function assertIncludes(haystack, needle, message = `${needle} should be present`) {
  assert.ok(haystack.includes(needle), message);
}

function assertDoesNotInclude(haystack, needle, message = `${needle} should not be present`) {
...
test("Pricing pages present credit purchase decision content honestly", () => {
  assert.ok(existsSync(outDir), "_site should exist before pricing content checks");

  const pricingHtml = readFileSync(path.join(outDir, "pricing", "index.html"), "utf8");
  const zhPricingHtml = readFileSync(path.join(outDir, "zh", "pricing", "index.html"), "utf8");
  ...
  for (const token of [
    ...
    assertIncludes(pricingHtml, token);
  }
  ...
  for (const token of [
    ...
    assertIncludes(zhPricingHtml, token);
  }

  for (const token of ["Buy now", "Start paid checkout"]) {
    assertDoesNotInclude(pricingHtml, token);
  }

  for (const token of ["立即付款", "立即购买额度"]) {
    assertDoesNotInclude(zhPricingHtml, token);
  }
});
