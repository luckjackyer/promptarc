...
assert.deepEqual(
  navLabels,
  ["Home", "Generate", "Gallery", "Pricing", "Account"],
  "homepage nav should include a clear home entry before the creation workflow"
);
...
assert.match(
  css,
  /body\[data-page="home-canvas"\] \.home-console-shortcuts a[\s\S]*min-height:\s*42px/,
  "homepage shortcut links should read as secondary CTA controls, not loose text links"
);

assert.match(
  css,
  /body\[data-page="home-canvas"\] \.home-console-shortcuts a[\s\S]*border-radius:\s*10px/,
  "homepage shortcut CTAs should use the same restrained control radius as the composer"
);
