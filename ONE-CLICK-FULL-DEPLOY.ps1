...
function Invoke-MojibakeAudit {
  Write-Step "Checking Chinese text encoding"
  & $nodePath "scripts\audit-chinese-mojibake.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Chinese text encoding audit failed."
  }
}

function Invoke-TrustSignalAudit {
  Write-Step "Checking trust signals"
  & $nodePath "scripts\audit-trust-signals.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Trust signal audit failed."
  }
}

function Invoke-PricingCreditAudit {
  Write-Step "Checking credit pricing page"
  & $nodePath "scripts\audit-pricing-credits.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Credit pricing audit failed."
  }
}

function Invoke-CommercialLinkAudit {
  Write-Step "Checking commercial links"
  & $nodePath "scripts\audit-commercial-links.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Commercial link audit failed."
  }
}

function Invoke-NumberedPromptProductAudit {
  Write-Step "Checking numbered prompt product"
  & $nodePath "scripts\build-numbered-prompt-catalog.mjs"
  if ($LASTEXITCODE -ne 0) {
    Fail "Building numbered prompt catalog failed."
  }
  & $nodePath "scripts\audit-numbered-prompt-product.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Numbered prompt product audit failed."
  }
}

function Invoke-SeoClusterAudit {
  Write-Step "Checking SEO clusters"
  & $nodePath "scripts\audit-seo-clusters.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "SEO cluster audit failed."
  }
}

function Invoke-TopicAnswerPageAudit {
  Write-Step "Checking topic answer pages"
  & $nodePath "scripts\audit-topic-answer-pages.mjs" $repoRoot
  if ($LASTEXITCODE -ne 0) {
    Fail "Topic answer page audit failed."
  }
}
...
try {
  ...
  Invoke-MojibakeAudit
  Invoke-TrustSignalAudit
  Invoke-PricingCreditAudit
  Invoke-CommercialLinkAudit
  Invoke-NumberedPromptProductAudit
  Invoke-StepCommand -Label "Reinforcing SEO clusters" -Command {
    & $nodePath "scripts\reinforce-seo-clusters.mjs" $repoRoot
  }
  Invoke-StepCommand -Label "Reinforcing topic answer pages" -Command {
    & $nodePath "scripts\reinforce-topic-answer-pages.mjs" $repoRoot
  }
  Invoke-SeoClusterAudit
  Invoke-TopicAnswerPageAudit
